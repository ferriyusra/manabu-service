pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/Mini-Soccer-Project/manabu-service.git'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/Mini-Soccer-Project/manabu-service.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "/home/faisalilhami/mini-soccer-project/manabu-service"
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:Mini-Soccer-Project/manabu-service.git "${targetDir}"
                cd "${targetDir}"
            fi
            ...
            sed -i "s/^CONSUL_HTTP_PATH=.*/CONSUL_HTTP_PATH=backend\\/manabu-service/" "${targetDir}/.env"
